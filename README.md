# Hi there! I'm JC Vadillo 👋
### Cybersecurity Specialist in Training | SOC & Incident Response Enthusiast 

Welcome to my GitHub profile! Here I document my hands-on projects, security labs, and professional certifications in the field of Cybersecurity.

---

## 🛠️ Certifications & Education
* **Google Cybersecurity Professional Certificate** 
  * 📁 [View Certificate Portfolio](https://github.com/JCVadillo/google-cybersecurity-cert-portfolio) — A comprehensive collection of practical coursework and hands-on labs covering network security, incident response frameworks (NIST CSF), log analysis, Python automation, SQL queries, and SIEM tools (Splunk/Chronicle).

---

## 🚀 Hands-On Practice & Labs
* 🎯 [TryHackMe Profile (jcvc87)](https://tryhackme.com/p/jcvc87) — Check out my active learning journey, completed rooms, badges, and practical cybersecurity exercises across offensive and defensive pathways.

---

## 💻 Technical Skills
* **Tools & Platforms:** Wireshark, Linux/Bash CLI, Git, SIEM platforms, Tcpdump.
* **Programming & Query Languages:** Python, Bash, SQL.
* **Frameworks & Concepts:** NIST CSF, OSI Model, Network Traffic Analysis, Threat Analysis.

---

📫 **Connect with me:**
* LinkedIn: [https://www.linkedin.com/in/juan-carlos-vadillo/](https://linkedin.com)
